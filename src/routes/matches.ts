...
router.put('/:id/result', authenticate, requireRole('admin', 'juez_sede'), async (req: AuthRequest, res: Response) => {
  const matchId = Number(req.params.id);
  const { setsA, setsB, pointsA, pointsB, isWO, woPlayerId, notes } = req.body;
  ...
  if (isWO) {
    // W.O.: the absent player loses, winner gets configured values
  ...
  // Close the match and free the table
